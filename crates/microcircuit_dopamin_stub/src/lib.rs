...
pub struct DopaRules {
    state: DopaState,
}

impl DopaRules {
    pub const UTILITY_MIN: i32 = -100;
    pub const UTILITY_MAX: i32 = 100;
    ...
    pub fn tick(&mut self, input: &DopaInput) -> DopaOutput {
...
#[derive(Debug, Clone, Default)]
pub struct DopaMicrocircuit {
    config: CircuitConfig,
    meta: CircuitStateMeta,
    rules: DopaRules,
}

impl DopaMicrocircuit {
    pub fn new(config: CircuitConfig) -> Self {
        Self {
            config,
            meta: CircuitStateMeta::default(),
            rules: DopaRules::default(),
        }
    }
}

impl MicrocircuitBackend<DopaInput, DopaOutput> for DopaMicrocircuit {
    fn step(&mut self, input: &DopaInput, now_ms: u64) -> DopaOutput {
    ...
    fn snapshot_digest(&self) -> [u8; 32] {
        ...
        bytes.extend(self.rules.state.utility_score.to_le_bytes());
        bytes.extend(self.rules.state.non_positive_streak.to_le_bytes());
        bytes.push(self.rules.state.reward_block as u8);

        digest_meta("UCF:MC:DOPA:STUB", &bytes)
    }

    fn config_digest(&self) -> [u8; 32] {
        digest_config("UCF:MC:DOPA:STUB:CONFIG", &self.config)
    }
}

impl DbmModule for DopaMicrocircuit {
...
mod tests {
    ...
    fn deterministic_outputs() {
        let mut module_a = DopaRules::new();
        let mut module_b = DopaRules::new();
        let input = DopaInput {
