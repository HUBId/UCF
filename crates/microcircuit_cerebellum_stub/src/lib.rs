...
impl DbmModule for CerebellumRules {
    type Input = CerInput;
...
mod tests {
    ...
    fn timeouts_trigger_high() {
        let mut cerebellum = CerebellumRules::new();
    ...
    fn bounded_tool_processing_and_recommendations() {
        ...
        assert!(output.tool_anomalies.len() <= CerebellumRules::MAX_TOOL_ANOMALIES);
        assert_eq!(
            output.suspend_recommendations.len(),
            CerebellumRules::MAX_SUSPEND_RECOMMENDATIONS
        );
