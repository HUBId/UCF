...
use crate::{
    ...
    models_active_review_snapshot, models_evidence_snapshot, prefix_hex, readiness_absolute_sweep,
    readiness_residual_sweep, readiness_terminal_sweep, reduce_reviewability,
    require_canonical_governance_entry, require_terminal_readiness_ultimate_inputs,
...
pub enum TerminalReadinessUltimateSweepStatusV1 {
    Pass,
    Fail,
    LegacyPresent,
}
...
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ReadinessUltimateMismatchCategoryV1 {
    ConsumerSkippedUltimateReadinessInputs,
    ConsumerUsedReadinessCachePath,
    ReadinessInputScopeMismatch,
    ReadinessInputSpineMismatch,
    ReadinessCachePathPresent,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ReadinessUltimateConsumerStatusV1 {
    pub consumer: String,
    pub status: TerminalReadinessUltimateSweepStatusV1,
    pub mismatch_categories: Vec<ReadinessUltimateMismatchCategoryV1>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct TerminalReadinessUltimateSweepV1 {
    ...
    pub sweep_status: TerminalReadinessUltimateSweepStatusV1,
    pub sweep_digest: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ReadinessUltimateSweepReportV1 {
    pub schema_version: u16,
    pub sweep: TerminalReadinessUltimateSweepV1,
    pub consumers: Vec<ReadinessUltimateConsumerStatusV1>,
}

pub fn readiness_ultimate_sweep(
    workdir: &Path,
    out: &Path,
) -> Result<ReadinessUltimateSweepReportV1, OpsError> {
    ...
    let active = models_active_review_snapshot(
        workdir,
        &workdir.join("out/active_review_snapshot_readiness_ultimate_sweep.json"),
    )?;
    ...
    let final_readiness = final_readiness_consumer_sweep(
        workdir,
        &workdir.join("out/final_readiness_consumer_sweep_readiness_ultimate_sweep.json"),
    )?;
    let residual = readiness_residual_sweep(
        workdir,
        &workdir.join("out/readiness_residual_sweep_readiness_ultimate_sweep.json"),
    )?;
    let residual_free = residual_free_readiness_sweep(
        workdir,
        &workdir.join("out/residual_free_readiness_sweep_readiness_ultimate_sweep.json"),
    )?;
    let absolute = readiness_absolute_sweep(
        workdir,
        &workdir.join("out/readiness_absolute_sweep_readiness_ultimate_sweep.json"),
    )?;
    let terminal = readiness_terminal_sweep(
        workdir,
        &workdir.join("out/readiness_terminal_sweep_readiness_ultimate_sweep.json"),
    )?;

    let authority_ctx = require_terminal_readiness_ultimate_inputs(
        &truths,
        Some(&reduction),
        &applied,
        &entry,
    ...
    let sweep_status = if consumers.iter().any(|c| {
        ...
        TerminalReadinessUltimateSweepStatusV1::Fail
    };
    ...
    let report = ReadinessUltimateSweepReportV1 {
        schema_version: 1,
        sweep,
        consumers,
    };
...
fn check_consumer(
    ...
    workdir: &Path,
    rel_path: &str,
    authority_ctx: &crate::TerminalReadinessUltimateInputsV1,
) -> Result<ReadinessUltimateConsumerStatusV1, OpsError> {
    let mut mismatch_categories = BTreeSet::new();
    let path = workdir.join(rel_path);
    if !path.exists() {
        mismatch_categories
            .insert(ReadinessUltimateMismatchCategoryV1::ConsumerSkippedUltimateReadinessInputs);
        return Ok(ReadinessUltimateConsumerStatusV1 {
            consumer: consumer.to_string(),
            status: TerminalReadinessUltimateSweepStatusV1::LegacyPresent,
    ...
    if !field_match("applied_supported_set_digest_prefix", expected_scope) {
        mismatch_categories
            .insert(ReadinessUltimateMismatchCategoryV1::ReadinessInputScopeMismatch);
    }
    if !field_match("canonical_readiness_spine_digest_prefix", expected_spine) {
        mismatch_categories
            .insert(ReadinessUltimateMismatchCategoryV1::ReadinessInputSpineMismatch);
    }
    if !field_match(
    ...
    if !field_match("readiness_residual_sweep_digest_prefix", expected_residual) {
        mismatch_categories.insert(ReadinessUltimateMismatchCategoryV1::ReadinessCachePathPresent);
    }
    if !field_match(
        ...
        mismatch_categories
            .insert(ReadinessUltimateMismatchCategoryV1::ConsumerUsedReadinessCachePath);
    }
    ...
    let status = if mismatch_categories.is_empty() {
        ...
        .contains(&ReadinessUltimateMismatchCategoryV1::ReadinessCachePathPresent)
        || mismatch_categories
            .contains(&ReadinessUltimateMismatchCategoryV1::ConsumerUsedReadinessCachePath)
    {
        TerminalReadinessUltimateSweepStatusV1::LegacyPresent
    } else {
        TerminalReadinessUltimateSweepStatusV1::Fail
    ...
    Ok(ReadinessUltimateConsumerStatusV1 {
        consumer: consumer.to_string(),
        status,
        mismatch_categories: mismatch_categories.into_iter().collect(),
    })
...
fn derive_sweep(
    ctx: &crate::TerminalReadinessUltimateInputsV1,
    covered_consumer_count: u16,
    residual_path_count: u16,
    sweep_status: TerminalReadinessUltimateSweepStatusV1,
) -> TerminalReadinessUltimateSweepV1 {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"terminal_readiness_ultimate_sweep_v1");
...
mod tests {
    ...
    #[test]
    fn readiness_ultimate_sweep_digest_stable() {
        let ctx = crate::TerminalReadinessUltimateInputsV1 {
        ...
        let first = derive_sweep(&ctx, 6, 0, TerminalReadinessUltimateSweepStatusV1::Pass);
        let second = derive_sweep(&ctx, 6, 0, TerminalReadinessUltimateSweepStatusV1::Pass);
        assert_eq!(first.sweep_digest, second.sweep_digest);
    }

    #[test]
    fn readiness_ultimate_status_is_deterministic_for_cache_mismatch() {
        let mismatches =
            BTreeSet::from([ReadinessUltimateMismatchCategoryV1::ReadinessCachePathPresent]);
