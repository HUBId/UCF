...
use crate::{
    ...
    primary_semantics_convergence_sweep, primary_semantics_residual_sweep,
    ...
    require_primary_semantics_final_consolidation_inputs, residual_free_primary_semantics_sweep,
    OpsError,
};

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum PrimarySemanticsFinalConsolidationStatusV1 {
    Pass,
    Fail,
    LegacyPresent,
}
...
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum PrimarySemanticsFinalConsolidationMismatchCategoryV1 {
    SurfaceSkippedPrimarySemanticsFinalConsolidation,
    SurfaceUsedPrimarySemanticsFacadePath,
    PrimaryBlockingOrderMismatch,
    PrimaryRemediationOrderMismatch,
    CanonicalConditionMappingMismatch,
    PrimarySemanticsFacadePathPresent,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct PrimarySemanticsFinalConsolidationSurfaceStatusV1 {
    pub surface: String,
    pub status: PrimarySemanticsFinalConsolidationStatusV1,
    pub mismatch_categories: Vec<PrimarySemanticsFinalConsolidationMismatchCategoryV1>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct PrimarySemanticsFinalConsolidationSweepV1 {
    ...
    pub consolidation_status: PrimarySemanticsFinalConsolidationStatusV1,
    pub consolidation_digest: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct PrimarySemanticsFinalConsolidationSweepReportV1 {
    pub schema_version: u16,
    pub sweep: PrimarySemanticsFinalConsolidationSweepV1,
    pub surfaces: Vec<PrimarySemanticsFinalConsolidationSurfaceStatusV1>,
}

pub fn primary_semantics_final_consolidation_sweep(
    workdir: &Path,
    out: &Path,
) -> Result<PrimarySemanticsFinalConsolidationSweepReportV1, OpsError> {
    let primary = primary_semantics_sweep(
        &workdir.join("out/primary_semantics_sweep_v18_final_consolidation.json"),
    )?;
    let final_sweep = final_primary_semantics_sweep(
        workdir,
        &workdir.join("out/final_primary_semantics_sweep_v18_final_consolidation.json"),
    )?;
    let residual = primary_semantics_residual_sweep(
        workdir,
        &workdir.join("out/primary_semantics_residual_sweep_v18_final_consolidation.json"),
    )?;
    let residual_free = residual_free_primary_semantics_sweep(
        workdir,
        &workdir.join("out/residual_free_primary_semantics_sweep_v18_final_consolidation.json"),
    )?;
    let absolute = primary_semantics_absolute_sweep(
        workdir,
        &workdir.join("out/primary_semantics_absolute_sweep_v18_final_consolidation.json"),
    )?;
    let terminal = primary_semantics_terminal_sweep(
        workdir,
        &workdir.join("out/primary_semantics_terminal_sweep_v18_final_consolidation.json"),
    )?;
    let ultimate = primary_semantics_ultimate_sweep(
        workdir,
        &workdir.join("out/primary_semantics_ultimate_sweep_v18_final_consolidation.json"),
    )?;
    let convergence = primary_semantics_convergence_sweep(
        workdir,
        &workdir.join("out/primary_semantics_convergence_sweep_v18_final_consolidation.json"),
    )?;
    let stabilization = primary_semantics_stabilization_sweep(
        workdir,
        &workdir.join("out/primary_semantics_stabilization_sweep_v18_final_consolidation.json"),
    )?;

    let authority_ctx = require_primary_semantics_final_consolidation_inputs(
        None,
        None,
    ...
    let mut surfaces = vec![
        check_surface(
            "GovernanceFinalConsolidationSweep",
            workdir,
            "out/governance_final_consolidation_sweep.json",
            &authority_ctx,
        )?,
        check_surface(
            "ReadinessFinalConsolidationSweep",
            workdir,
            "out/readiness_final_consolidation_sweep.json",
            &authority_ctx,
        )?,
        check_surface(
            "BundleFinalConsolidationSweep",
            workdir,
    ...
    let consolidation_status = if surfaces.iter().any(|s| {
        matches!(
            s.status,
        ...
        PrimarySemanticsFinalConsolidationStatusV1::Fail
    };
    ...
    let report = PrimarySemanticsFinalConsolidationSweepReportV1 {
        schema_version: 1,
        sweep,
        surfaces,
    };
...
fn check_surface(
    ...
    workdir: &Path,
    rel_path: &str,
    authority_ctx: &crate::PrimarySemanticsFinalConsolidationInputsV1,
) -> Result<PrimarySemanticsFinalConsolidationSurfaceStatusV1, OpsError> {
    let mut mismatch_categories = BTreeSet::new();
    let path = workdir.join(rel_path);
    if !path.exists() {
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::SurfaceSkippedPrimarySemanticsFinalConsolidation,
        );
        return Ok(PrimarySemanticsFinalConsolidationSurfaceStatusV1 {
            surface: surface.to_string(),
            status: PrimarySemanticsFinalConsolidationStatusV1::LegacyPresent,
    ...
    if !field_match(
        ...
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::PrimarySemanticsFacadePathPresent,
        );
    }
    if !field_match(
        ...
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::SurfaceUsedPrimarySemanticsFacadePath,
        );
    }
    if !field_match(
        ...
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::PrimaryBlockingOrderMismatch,
        );
    }
    if !field_match(
        ...
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::PrimaryRemediationOrderMismatch,
        );
    }
    if !field_match(
        ...
        mismatch_categories.insert(
            PrimarySemanticsFinalConsolidationMismatchCategoryV1::CanonicalConditionMappingMismatch,
        );
    }
    ...
    Ok(PrimarySemanticsFinalConsolidationSurfaceStatusV1 {
        surface: surface.to_string(),
        status,
        mismatch_categories: mismatch_categories.into_iter().collect(),
    })
...
fn derive_sweep(
    ctx: &crate::PrimarySemanticsFinalConsolidationInputsV1,
    covered_surface_count: u16,
    residual_path_count: u16,
    consolidation_status: PrimarySemanticsFinalConsolidationStatusV1,
) -> PrimarySemanticsFinalConsolidationSweepV1 {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"primary_semantics_final_consolidation_sweep_v1");
...
mod tests {
    ...
    #[test]
    fn primary_semantics_final_consolidation_digest_stable() {
        let ctx = crate::PrimarySemanticsFinalConsolidationInputsV1 {
        ...
        let first = derive_sweep(&ctx, 8, 0, PrimarySemanticsFinalConsolidationStatusV1::Pass);
        let second = derive_sweep(&ctx, 8, 0, PrimarySemanticsFinalConsolidationStatusV1::Pass);
