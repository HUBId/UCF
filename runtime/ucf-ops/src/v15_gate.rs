...
use crate::{
    artifact_schema, bundle_ultimate_sweep, check_artifact_schema_snapshots, docs_lint,
    governance_ultimate_sweep, load_applied_supported_set_context_v1, models_consistency_check,
    portability_check, primary_semantics_ultimate_sweep, readiness_ultimate_sweep,
    ultimate_terminal_absolute_final_input_continuity_sweep, v0_gate, v10_gate, v11_gate, v12_gate,
    v13_gate, v14_gate, v1_gate, v2_gate, v3_gate, v4_gate, v5_gate, v6_gate, v7_gate, v8_gate,
    v9_gate, DocsLintArgs, DocsLintMode, GateStatus, OpsError, SupportedScopeExecutionV10,
    ...
    UltimateTerminalAbsoluteFinalInputContinuityStatusV1, V0GateOverallStatus,
    V10GateOverallStatus, V11GateOverallStatus, V12GateOverallStatus, V13GateOverallStatus,
    V14GateOverallStatus, V1GateOverallStatus, V2GateOverallStatus, V3GateOverallStatus,
...
pub enum V15GateOverallStatus {
    Pass,
    Fail,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V15GateCheckV1 {
    pub name: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V15GateReportV1 {
    pub schema_version: u16,
    pub overall_status: V15GateOverallStatus,
    pub checks: Vec<V15GateCheckV1>,
}

pub fn v15_gate(workdir: &Path, out: &Path) -> Result<V15GateReportV1, OpsError> {
    ...
    let v0 = v0_gate(
        ...
        &workdir.join("out/v0_gate_report_v15_gate.json"),
    )?;
    checks.push(v15_gate_check(
        "v0_gate_pass",
    ...
    let v1 = v1_gate(workdir, &workdir.join("out/v1_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v1_gate_pass",
    ...
    let v2 = v2_gate(workdir, &workdir.join("out/v2_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v2_gate_pass",
    ...
    let v3 = v3_gate(workdir, &workdir.join("out/v3_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v3_gate_pass",
    ...
    let v4 = v4_gate(workdir, &workdir.join("out/v4_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v4_gate_pass",
    ...
    let v5 = v5_gate(workdir, &workdir.join("out/v5_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v5_gate_pass",
    ...
    let v6 = v6_gate(workdir, &workdir.join("out/v6_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v6_gate_pass",
    ...
    let v7 = v7_gate(workdir, &workdir.join("out/v7_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v7_gate_pass",
    ...
    let v8 = v8_gate(workdir, &workdir.join("out/v8_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v8_gate_pass",
    ...
    let v9 = v9_gate(workdir, &workdir.join("out/v9_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v9_gate_pass",
    ...
    let v10 = v10_gate(workdir, &workdir.join("out/v10_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v10_gate_pass",
    ...
    let v11 = v11_gate(workdir, &workdir.join("out/v11_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v11_gate_pass",
    ...
    let v12 = v12_gate(workdir, &workdir.join("out/v12_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v12_gate_pass",
    ...
    let v13 = v13_gate(workdir, &workdir.join("out/v13_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v13_gate_pass",
    ...
    let v14 = v14_gate(workdir, &workdir.join("out/v14_gate_report_v15_gate.json"))?;
    checks.push(v15_gate_check(
        "v14_gate_pass",
    ...
    let governance = governance_ultimate_sweep(
        workdir,
        &workdir.join("out/governance_ultimate_sweep_v15_gate.json"),
    )?;
    checks.push(v15_gate_check(
        "ultimate_terminal_absolute_residual_free_final_governance_inputs_pass",
        gate_from_bool(matches!(
            governance.sweep.sweep_status,
            TerminalGovernanceUltimateSweepStatusV1::Pass
        )),
        [(
            "governance_ultimate_sweep".to_string(),
            digest_prefix(&governance)?,
        )],
        "REMEDIATE_GOVERNANCE_ULTIMATE_SWEEP",
        "NOTE_REQUIRED_ULTIMATE_GOVERNANCE",
    ));

    let supported_scope_execution_path = workdir.join("out/supported_scope_execute_v10.json");
    let supported_scope_execution =
        load_supported_scope_execution_v10(&supported_scope_execution_path)?;
    checks.push(v15_gate_check(
        "supported_scope_execution_v10_present",
        gate_from_bool(supported_scope_execution.is_some()),
        [(
            "supported_scope_execution_v10".to_string(),
            file_digest_or_missing(&supported_scope_execution_path)?,
        )],
        "REMEDIATE_RUN_SUPPORTED_SCOPE_EXECUTION_V10",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V10",
    ));
    ...
    checks.push(v15_gate_check(
        "supported_scope_execution_v10_consistent",
        ...
        "REMEDIATE_SUPPORTED_SCOPE_EXECUTION_V10_STALE",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V10_CURRENT",
    ));

    let readiness = readiness_ultimate_sweep(
        workdir,
        &workdir.join("out/readiness_ultimate_sweep_v15_gate.json"),
    )?;
    checks.push(v15_gate_check(
        "ultimate_terminal_absolute_residual_free_final_readiness_inputs_pass",
        gate_from_bool(matches!(
            readiness.sweep.sweep_status,
            TerminalReadinessUltimateSweepStatusV1::Pass
        )),
        [(
            "readiness_ultimate_sweep".to_string(),
            digest_prefix(&readiness)?,
        )],
        "REMEDIATE_READINESS_ULTIMATE_SWEEP",
        "NOTE_REQUIRED_ULTIMATE_READINESS",
    ));

    let bundle = bundle_ultimate_sweep(
        workdir,
        &workdir.join("out/bundle_ultimate_sweep_v15_gate.json"),
    )?;
    checks.push(v15_gate_check(
        "ultimate_terminal_absolute_residual_free_final_bundle_inputs_pass",
        gate_from_bool(matches!(
            bundle.sweep.sweep_status,
            TerminalBundleUltimateSweepStatusV1::Pass
        )),
        [("bundle_ultimate_sweep".to_string(), digest_prefix(&bundle)?)],
        "REMEDIATE_BUNDLE_ULTIMATE_SWEEP",
        "NOTE_REQUIRED_ULTIMATE_BUNDLE",
    ));

    let semantics = primary_semantics_ultimate_sweep(
        workdir,
        &workdir.join("out/primary_semantics_ultimate_sweep_v15_gate.json"),
    )?;
    checks.push(v15_gate_check(
        "ultimate_terminal_absolute_residual_free_final_primary_semantics_inputs_pass",
        gate_from_bool(matches!(
            semantics.sweep.sweep_status,
            TerminalPrimarySemanticsUltimateSweepStatusV1::Pass
        )),
        [(
            "primary_semantics_ultimate_sweep".to_string(),
            digest_prefix(&semantics)?,
        )],
        "REMEDIATE_PRIMARY_SEMANTICS_ULTIMATE_SWEEP",
        "NOTE_REQUIRED_ULTIMATE_PRIMARY_SEMANTICS",
    ));
    ...
    let continuity = continuity_bundle
        ...
        .map(|bundle_path| {
            ultimate_terminal_absolute_final_input_continuity_sweep(
                ...
                bundle_path,
                &workdir.join(
                    "out/ultimate_terminal_absolute_final_input_continuity_sweep_v15_gate.json",
                ),
            )
        })
        .transpose()?;
    ...
    checks.push(v15_gate_check(
        "sole_ultimate_terminal_absolute_residual_free_final_input_top_level_continuity_proof_pass",
        continuity_status,
        [(
    ...
    let portability = portability_check(&workdir.join("out/portability_check_v15_gate.json"))?;
    checks.push(v15_gate_check(
    ...
    let model_consistency = models_consistency_check(
        workdir,
        &workdir.join("out/models_consistency_check_v15_gate.json"),
    )?;
...
fn legacy_status_from_governance(report: &crate::GovernanceUltimateSweepReportV1) -> GateStatus {
    if matches!(
...
fn legacy_status_from_readiness(report: &crate::ReadinessUltimateSweepReportV1) -> GateStatus {
    if matches!(
        report.sweep.sweep_status,
        TerminalReadinessUltimateSweepStatusV1::LegacyPresent
    ) {
...
fn legacy_status_from_bundle(report: &crate::BundleUltimateSweepReportV1) -> GateStatus {
    if matches!(
        report.sweep.sweep_status,
        TerminalBundleUltimateSweepStatusV1::LegacyPresent
    ) {
...
fn load_supported_scope_execution_v10(
    path: &Path,
) -> Result<Option<SupportedScopeExecutionV10>, OpsError> {
    if !path.exists() {
        return Ok(None);
    }
...
fn overall_from_checks(checks: &[V15GateCheckV1]) -> V15GateOverallStatus {
    if checks
        .iter()
        .all(|check| matches!(check.status, GateStatus::Pass | GateStatus::Skip))
    {
        V15GateOverallStatus::Pass
    } else {
        V15GateOverallStatus::Fail
    }
}
...
mod tests {
    ...
    #[test]
    fn v15_gate_check_order_is_fixed() {
        let checks = vec![
            "v0_gate_pass",
            "v1_gate_pass",
            "v2_gate_pass",
        ...
        let report = V15GateReportV1 {
            schema_version: 1,
            overall_status: V15GateOverallStatus::Pass,
            checks: checks
                .iter()
                .map(|name| v15_gate_check(name, GateStatus::Pass, [], "REMEDIATE", "NOTE"))
                .collect(),
        };
    ...
    #[test]
    fn v15_gate_report_serialization_is_deterministic() {
        let report = V15GateReportV1 {
            schema_version: 1,
            overall_status: V15GateOverallStatus::Pass,
            checks: vec![
    ...
    #[test]
    fn v15_gate_normalization_is_fail_closed() {
        let checks = vec![
            v15_gate_check("required", GateStatus::Pass, [], "R", "N"),
            v15_gate_check("required_fail", GateStatus::Fail, [], "R", "N"),
            v15_gate_check("optional_skip", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
    ...
    #[test]
    fn v15_gate_normalization_supports_optional_skip_on_pass_path() {
        let checks = vec![
            v15_gate_check("required_a", GateStatus::Pass, [], "R", "N"),
            v15_gate_check("required_b", GateStatus::Pass, [], "R", "N"),
            v15_gate_check("optional", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
            overall_from_checks(&checks),
            V15GateOverallStatus::Pass
    ...
    #[test]
    fn supported_scope_execution_v10_missing_is_none() {
        let temp = tempfile::tempdir().expect("tmp");
        let missing = temp.path().join("supported_scope_execute_v10.json");
        let loaded = load_supported_scope_execution_v10(&missing).expect("load");
        assert!(loaded.is_none());
    }

    #[test]
    fn required_ultimate_checks_fail_normalization_cases() {
        for required in [
            "supported_scope_execution_v10_present",
            "ultimate_terminal_absolute_residual_free_final_readiness_inputs_pass",
            "ultimate_terminal_absolute_residual_free_final_bundle_inputs_pass",
            ...
            let checks = vec![
                v15_gate_check("v14_gate_pass", GateStatus::Pass, [], "R", "N"),
                v15_gate_check(required, GateStatus::Fail, [], "R", "N"),
                v15_gate_check(
                    "optional_backend_path_consistent",
                    GateStatus::Skip,
                    [],
