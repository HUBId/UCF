...
use crate::{
    bundle_absolute_sweep, bundle_convergence_sweep, bundle_residual_sweep, bundle_terminal_sweep,
    bundle_ultimate_sweep, derive_canonical_bundle_authority_v2, derive_canonical_governance_entry,
    ...
    require_bundle_stabilization_inputs, require_canonical_governance_entry,
...
pub enum BundleStabilizationStatusV1 {
    Pass,
    Fail,
    LegacyPresent,
}
...
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum BundleStabilizationMismatchCategoryV1 {
    ConsumerSkippedBundleStabilization,
    ConsumerUsedBundleAdapterPath,
    BundleInputScopeMismatch,
    BundleInputSpineMismatch,
    BundleInputExportContextMismatch,
    BundleAdapterPathPresent,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct BundleStabilizationConsumerStatusV1 {
    pub consumer: String,
    pub status: BundleStabilizationStatusV1,
    pub mismatch_categories: Vec<BundleStabilizationMismatchCategoryV1>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct BundleStabilizationSweepV1 {
    ...
    pub stabilization_status: BundleStabilizationStatusV1,
    pub stabilization_digest: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct BundleStabilizationSweepReportV1 {
    pub schema_version: u16,
    pub sweep: BundleStabilizationSweepV1,
    pub consumers: Vec<BundleStabilizationConsumerStatusV1>,
}

pub fn bundle_stabilization_sweep(
    workdir: &Path,
    out: &Path,
) -> Result<BundleStabilizationSweepReportV1, OpsError> {
    ...
    let active = models_active_review_snapshot(
        workdir,
        &workdir.join("out/active_review_snapshot_bundle_stabilization_sweep.json"),
    )?;
    let surfaces =
    ...
    let readiness = readiness_spine_check(
        workdir,
        &workdir.join("out/readiness_spine_check_bundle_stabilization_sweep.json"),
    )?
    .canonical_readiness_spine;
    let final_bundle_consumer = final_bundle_consumer_sweep(
        workdir,
        &workdir.join("out/final_bundle_consumer_sweep_bundle_stabilization.json"),
    )?;
    let final_bundle_residual = bundle_residual_sweep(
        workdir,
        &workdir.join("out/bundle_residual_sweep_bundle_stabilization.json"),
    )?;
    let residual_free = residual_free_bundle_sweep(
        workdir,
        &workdir.join("out/residual_free_bundle_sweep_bundle_stabilization.json"),
    )?;
    let absolute = bundle_absolute_sweep(
        workdir,
        &workdir.join("out/bundle_absolute_sweep_bundle_stabilization.json"),
    )?;
    let terminal = bundle_terminal_sweep(
        workdir,
        &workdir.join("out/bundle_terminal_sweep_bundle_stabilization.json"),
    )?;
    let ultimate = bundle_ultimate_sweep(
        workdir,
        &workdir.join("out/bundle_ultimate_sweep_bundle_stabilization.json"),
    )?;
    let convergence = bundle_convergence_sweep(
        workdir,
        &workdir.join("out/bundle_convergence_sweep_bundle_stabilization.json"),
    )?;

    let run_id = crate::reproducible_run_id_for_smoke(workdir)?;
    let repro_bundle = workdir
        .join("out")
        .join(format!("repro_{run_id}_bundle_stabilization_sweep.zip"));
    let _ = repro_pack(workdir, &run_id, &repro_bundle)?;
    let repro_spine = exports_bundle_spine_check(
        &repro_bundle,
        &workdir.join("out/repro_bundle_spine_check_bundle_stabilization_sweep.json"),
    )?;
    ...
    let authority_ctx = require_bundle_stabilization_inputs(
        CanonicalBundleKindV1::Repro,
    ...
    let bugkit_bundle = workdir
        .join("out")
        .join(format!("bugkit_{run_id}_bundle_stabilization_sweep.zip"));
    ...
    let residual_path_count = consumers
        .iter()
        .filter(|consumer| !matches!(consumer.status, BundleStabilizationStatusV1::Pass))
        .count() as u16;

    let stabilization_status = if consumers
        .iter()
        .any(|c| matches!(c.status, BundleStabilizationStatusV1::LegacyPresent))
    {
        BundleStabilizationStatusV1::LegacyPresent
    } else if residual_path_count == 0 {
        BundleStabilizationStatusV1::Pass
    } else {
        BundleStabilizationStatusV1::Fail
    };
    ...
    let report = BundleStabilizationSweepReportV1 {
        schema_version: 1,
        sweep,
        consumers,
    };
...
fn check_consumer(
    ...
    workdir: &Path,
    rel_path: &str,
    authority_ctx: &crate::BundleStabilizationInputsV1,
) -> Result<BundleStabilizationConsumerStatusV1, OpsError> {
    let mut mismatch_categories = BTreeSet::new();
    let path = workdir.join(rel_path);
    if !path.exists() {
        mismatch_categories
            .insert(BundleStabilizationMismatchCategoryV1::ConsumerSkippedBundleStabilization);
        return Ok(BundleStabilizationConsumerStatusV1 {
            consumer: consumer.to_string(),
            status: BundleStabilizationStatusV1::LegacyPresent,
    ...
    if !field_match(
        ...
        mismatch_categories.insert(BundleStabilizationMismatchCategoryV1::BundleInputScopeMismatch);
    }
    if !field_match(
        "canonical_bundle_spine_digest_prefix",
        &authority_ctx.canonical_bundle_spine_digest_prefix,
    ) {
        mismatch_categories.insert(BundleStabilizationMismatchCategoryV1::BundleInputSpineMismatch);
    }
    if !field_match(
        "bundle_convergence_sweep_digest_prefix",
        &authority_ctx.bundle_convergence_sweep_digest_prefix,
    ) {
        mismatch_categories
            .insert(BundleStabilizationMismatchCategoryV1::ConsumerSkippedBundleStabilization);
    }
    if value.get("bundle_roundtrip_digest").is_some() || value.get("roundtrip_digest").is_some() {
        mismatch_categories.insert(BundleStabilizationMismatchCategoryV1::BundleAdapterPathPresent);
    }
    if value.get("bundle_adapter_path").is_some() || value.get("bundle_translation_path").is_some()
    {
        mismatch_categories
            .insert(BundleStabilizationMismatchCategoryV1::ConsumerUsedBundleAdapterPath);
    }
    if value
        .get("export_context")
    ...
    {
        mismatch_categories
            .insert(BundleStabilizationMismatchCategoryV1::BundleInputExportContextMismatch);
    }

    let status = if mismatch_categories.is_empty() {
        BundleStabilizationStatusV1::Pass
    } else if mismatch_categories
        .contains(&BundleStabilizationMismatchCategoryV1::BundleAdapterPathPresent)
        || mismatch_categories
            .contains(&BundleStabilizationMismatchCategoryV1::ConsumerUsedBundleAdapterPath)
    {
        BundleStabilizationStatusV1::LegacyPresent
    } else {
        BundleStabilizationStatusV1::Fail
    ...
    Ok(BundleStabilizationConsumerStatusV1 {
        consumer: consumer.to_string(),
        status,
        mismatch_categories: mismatch_categories.into_iter().collect(),
    })
...
fn derive_sweep(
    ctx: &crate::BundleStabilizationInputsV1,
    covered_consumer_count: u16,
    residual_path_count: u16,
    stabilization_status: BundleStabilizationStatusV1,
) -> BundleStabilizationSweepV1 {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"bundle_stabilization_sweep_v1");
...
mod tests {
    ...
    #[test]
    fn bundle_stabilization_sweep_digest_stable() {
        let ctx = crate::BundleStabilizationInputsV1 {
        ...
        let first = derive_sweep(&ctx, 7, 0, BundleStabilizationStatusV1::Pass);
        let second = derive_sweep(&ctx, 7, 0, BundleStabilizationStatusV1::Pass);
        assert_eq!(first.stabilization_digest, second.stabilization_digest);
    }

    #[test]
    fn bundle_stabilization_status_deterministic_for_adapter_paths() {
        let mismatches =
            BTreeSet::from([BundleStabilizationMismatchCategoryV1::BundleAdapterPathPresent]);
        ...
        assert_eq!(status, BundleStabilizationStatusV1::LegacyPresent);
    }
}
