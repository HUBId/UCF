...
use crate::{
    artifact_schema, bundle_closure_sweep, canonical_closure_continuity_sweep,
    check_artifact_schema_snapshots, docs_lint, governance_closure_sweep,
    load_applied_supported_set_context_v1, models_consistency_check, portability_check,
    primary_semantics_closure_sweep, readiness_closure_sweep, v0_gate, v10_gate, v11_gate,
    v12_gate, v13_gate, v14_gate, v15_gate, v16_gate, v17_gate, v18_gate, v1_gate, v2_gate,
    v3_gate, v4_gate, v5_gate, v6_gate, v7_gate, v8_gate, v9_gate, BundleClosureStatusV1,
    CanonicalClosureContinuityStatusV1, DocsLintArgs, DocsLintMode, GateStatus,
    GovernanceClosureStatusV1, OpsError, PrimarySemanticsClosureStatusV1, ReadinessClosureStatusV1,
    SupportedScopeExecutionV14, V0GateOverallStatus, V10GateOverallStatus, V11GateOverallStatus,
    V12GateOverallStatus, V13GateOverallStatus, V14GateOverallStatus, V15GateOverallStatus,
    V16GateOverallStatus, V17GateOverallStatus, V18GateOverallStatus, V1GateOverallStatus,
...
pub enum V19GateOverallStatus {
    Pass,
    Fail,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V19GateCheckV1 {
    pub name: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V19GateReportV1 {
    pub schema_version: u16,
    pub overall_status: V19GateOverallStatus,
    pub checks: Vec<V19GateCheckV1>,
}

pub fn v19_gate(workdir: &Path, out: &Path) -> Result<V19GateReportV1, OpsError> {
    ...
    let v0 = v0_gate(
        ...
        &workdir.join("out/v0_gate_report_v19_gate.json"),
    )?;
    checks.push(v19_gate_check(
        "v0_gate_pass",
    ...
    let v1 = v1_gate(workdir, &workdir.join("out/v1_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v1_gate_pass",
    ...
    let v2 = v2_gate(workdir, &workdir.join("out/v2_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v2_gate_pass",
    ...
    let v3 = v3_gate(workdir, &workdir.join("out/v3_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v3_gate_pass",
    ...
    let v4 = v4_gate(workdir, &workdir.join("out/v4_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v4_gate_pass",
    ...
    let v5 = v5_gate(workdir, &workdir.join("out/v5_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v5_gate_pass",
    ...
    let v6 = v6_gate(workdir, &workdir.join("out/v6_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v6_gate_pass",
    ...
    let v7 = v7_gate(workdir, &workdir.join("out/v7_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v7_gate_pass",
    ...
    let v8 = v8_gate(workdir, &workdir.join("out/v8_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v8_gate_pass",
    ...
    let v9 = v9_gate(workdir, &workdir.join("out/v9_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v9_gate_pass",
    ...
    let v10 = v10_gate(workdir, &workdir.join("out/v10_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v10_gate_pass",
    ...
    let v11 = v11_gate(workdir, &workdir.join("out/v11_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v11_gate_pass",
    ...
    let v12 = v12_gate(workdir, &workdir.join("out/v12_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v12_gate_pass",
    ...
    let v13 = v13_gate(workdir, &workdir.join("out/v13_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v13_gate_pass",
    ...
    let v14 = v14_gate(workdir, &workdir.join("out/v14_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v14_gate_pass",
    ...
    let v15 = v15_gate(workdir, &workdir.join("out/v15_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v15_gate_pass",
    ...
    let v16 = v16_gate(workdir, &workdir.join("out/v16_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v16_gate_pass",
    ...
    let v17 = v17_gate(workdir, &workdir.join("out/v17_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v17_gate_pass",
    ...
    let v18 = v18_gate(workdir, &workdir.join("out/v18_gate_report_v19_gate.json"))?;
    checks.push(v19_gate_check(
        "v18_gate_pass",
    ...
    let governance = governance_closure_sweep(
        workdir,
        &workdir.join("out/governance_closure_sweep_v19_gate.json"),
    )?;
    checks.push(v19_gate_check(
        "governance_closure_pass",
        gate_from_bool(matches!(
            governance.sweep.closure_status,
            GovernanceClosureStatusV1::Pass
        )),
        [(
            "governance_closure_sweep".to_string(),
            digest_prefix(&governance)?,
        )],
        "REMEDIATE_GOVERNANCE_CLOSURE_SWEEP",
        "NOTE_REQUIRED_GOVERNANCE_CLOSURE",
    ));

    let supported_scope_execution_path = workdir.join("out/supported_scope_execute_v14.json");
    let supported_scope_execution =
        load_supported_scope_execution_v14(&supported_scope_execution_path)?;
    checks.push(v19_gate_check(
        "supported_scope_execution_v14_present",
        gate_from_bool(supported_scope_execution.is_some()),
        [(
            "supported_scope_execution_v14".to_string(),
            file_digest_or_missing(&supported_scope_execution_path)?,
        )],
        "REMEDIATE_RUN_SUPPORTED_SCOPE_EXECUTION_V14",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V14",
    ));
    ...
    let supported_scope_consistent = supported_scope_execution.as_ref().is_some_and(|exec| {
        exec.resulting_supported_set_digest_prefix == applied_scope.applied_set_digest_prefix
            ...
            && exec.governance_closure_sweep_digest_prefix
                == prefix_hex(&governance.sweep.closure_digest)
    });
    checks.push(v19_gate_check(
        "supported_scope_execution_v14_consistent",
        ...
        "REMEDIATE_SUPPORTED_SCOPE_EXECUTION_V14_STALE",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V14_CURRENT",
    ));

    let readiness = readiness_closure_sweep(
        workdir,
        &workdir.join("out/readiness_closure_sweep_v19_gate.json"),
    )?;
    checks.push(v19_gate_check(
        "readiness_closure_pass",
        gate_from_bool(matches!(
            readiness.sweep.closure_status,
            ReadinessClosureStatusV1::Pass
        )),
        [(
            "readiness_closure_sweep".to_string(),
            digest_prefix(&readiness)?,
        )],
        "REMEDIATE_READINESS_CLOSURE_SWEEP",
        "NOTE_REQUIRED_READINESS_CLOSURE",
    ));

    let bundle = bundle_closure_sweep(
        workdir,
        &workdir.join("out/bundle_closure_sweep_v19_gate.json"),
    )?;
    checks.push(v19_gate_check(
        "bundle_closure_pass",
        gate_from_bool(matches!(
            bundle.sweep.closure_status,
            BundleClosureStatusV1::Pass
        )),
        [("bundle_closure_sweep".to_string(), digest_prefix(&bundle)?)],
        "REMEDIATE_BUNDLE_CLOSURE_SWEEP",
        "NOTE_REQUIRED_BUNDLE_CLOSURE",
    ));

    let semantics = primary_semantics_closure_sweep(
        workdir,
        &workdir.join("out/primary_semantics_closure_sweep_v19_gate.json"),
    )?;
    checks.push(v19_gate_check(
        "primary_semantics_closure_pass",
        gate_from_bool(matches!(
            semantics.sweep.closure_status,
            PrimarySemanticsClosureStatusV1::Pass
        )),
        [(
            "primary_semantics_closure_sweep".to_string(),
            digest_prefix(&semantics)?,
        )],
        "REMEDIATE_PRIMARY_SEMANTICS_CLOSURE_SWEEP",
        "NOTE_REQUIRED_PRIMARY_SEMANTICS_CLOSURE",
    ));
    ...
    let continuity = continuity_bundle
        .as_ref()
        .map(|bundle_path| {
            canonical_closure_continuity_sweep(
                workdir,
                bundle_path,
                &workdir.join("out/canonical_closure_continuity_sweep_v19_gate.json"),
            )
        })
        .transpose()?;
    ...
    checks.push(v19_gate_check(
        "sole_closure_complete_canonical_top_level_continuity_proof_pass",
        continuity_status,
        [(
        ...
        "REMEDIATE_CANONICAL_CLOSURE_CONTINUITY",
        "NOTE_REQUIRED_CANONICAL_CLOSURE_CONTINUITY",
    ));
    ...
    let portability = portability_check(&workdir.join("out/portability_check_v19_gate.json"))?;
    checks.push(v19_gate_check(
    ...
    let model_consistency = models_consistency_check(
        workdir,
        &workdir.join("out/models_consistency_check_v19_gate.json"),
    )?;
...
fn legacy_status_from_governance(report: &crate::GovernanceClosureSweepReportV1) -> GateStatus {
    if matches!(
...
fn legacy_status_from_readiness(report: &crate::ReadinessClosureSweepReportV1) -> GateStatus {
    if matches!(
        report.sweep.closure_status,
        ReadinessClosureStatusV1::LegacyPresent
    ) {
...
fn legacy_status_from_bundle(report: &crate::BundleClosureSweepReportV1) -> GateStatus {
    if matches!(
        report.sweep.closure_status,
        BundleClosureStatusV1::LegacyPresent
    ) {
...
fn load_supported_scope_execution_v14(
    path: &Path,
) -> Result<Option<SupportedScopeExecutionV14>, OpsError> {
    if !path.exists() {
        return Ok(None);
    }
...
fn overall_from_checks(checks: &[V19GateCheckV1]) -> V19GateOverallStatus {
    if checks
        .iter()
        .all(|check| matches!(check.status, GateStatus::Pass | GateStatus::Skip))
    {
        V19GateOverallStatus::Pass
    } else {
        V19GateOverallStatus::Fail
    }
}
...
mod tests {
    ...
    #[test]
    fn v19_gate_check_order_is_fixed() {
        let checks = vec![
            "v0_gate_pass",
            "v1_gate_pass",
            "v2_gate_pass",
        ...
        let report = V19GateReportV1 {
            schema_version: 1,
            overall_status: V19GateOverallStatus::Pass,
            checks: checks
                .iter()
                .map(|name| v19_gate_check(name, GateStatus::Pass, [], "REMEDIATE", "NOTE"))
                .collect(),
        };
    ...
    #[test]
    fn v19_gate_report_serialization_is_deterministic() {
        let report = V19GateReportV1 {
            schema_version: 1,
            overall_status: V19GateOverallStatus::Pass,
            checks: vec![
    ...
    #[test]
    fn v19_gate_normalization_is_fail_closed() {
        let checks = vec![
            v19_gate_check("required", GateStatus::Pass, [], "R", "N"),
            v19_gate_check("required_fail", GateStatus::Fail, [], "R", "N"),
            v19_gate_check("optional_skip", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
    ...
    #[test]
    fn v19_gate_normalization_supports_optional_skip_on_pass_path() {
        let checks = vec![
            v19_gate_check("required_a", GateStatus::Pass, [], "R", "N"),
            v19_gate_check("required_b", GateStatus::Pass, [], "R", "N"),
            v19_gate_check("optional", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
            overall_from_checks(&checks),
            V19GateOverallStatus::Pass
    ...
    #[test]
    fn supported_scope_execution_v14_missing_is_none() {
        let temp = tempfile::tempdir().expect("tmp");
        let missing = temp.path().join("supported_scope_execute_v14.json");
        let loaded = load_supported_scope_execution_v14(&missing).expect("load");
        assert!(loaded.is_none());
    }

    #[test]
    fn required_closure_checks_fail_normalization_cases() {
        for required in [
            "supported_scope_execution_v14_present",
            "readiness_closure_pass",
            "bundle_closure_pass",
            ...
            let checks = vec![
                v19_gate_check("v18_gate_pass", GateStatus::Pass, [], "R", "N"),
                v19_gate_check(required, GateStatus::Fail, [], "R", "N"),
                v19_gate_check(
                    "optional_backend_path_consistent",
                    GateStatus::Skip,
                    [],
