...
pub fn models_supported_set_apply(
    ...
    let execution = ensure_current_supported_scope_execution_v5(workdir, &policy, &previous_set)?;
...
fn ensure_current_supported_scope_execution_v5(
    workdir: &Path,
    policy: &SupportedRealSlotSetPolicyV2,
    previous_set: &SupportedRealSlotSetV1,
) -> Result<SupportedScopeExecutionV5, OpsError> {
    let reeval = ensure_current_supported_scope_reevaluation_v1(workdir, policy, previous_set)?;
    let path = workdir.join("out").join("supported_scope_execute_v5.json");
    if let Ok(report) = read_json_file::<SupportedScopeExecutionV5>(&path) {
    ...
    models_supported_scope_execute_v5(workdir, &path)
}
