...
use crate::{
    ...
    readiness_residual_sweep, reduce_reviewability,
    require_absolute_final_readiness_terminal_inputs, require_canonical_governance_entry,
...
pub enum AbsoluteFinalReadinessTerminalSweepStatusV1 {
    Pass,
    Fail,
    LegacyPresent,
}
...
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum ReadinessTerminalMismatchCategoryV1 {
    ConsumerSkippedTerminalReadinessInputs,
    ConsumerUsedReadinessEchoPath,
    ReadinessInputScopeMismatch,
    ReadinessInputSpineMismatch,
    ReadinessEchoPathPresent,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ReadinessTerminalConsumerStatusV1 {
    pub consumer: String,
    pub status: AbsoluteFinalReadinessTerminalSweepStatusV1,
    pub mismatch_categories: Vec<ReadinessTerminalMismatchCategoryV1>,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct AbsoluteFinalReadinessTerminalSweepV1 {
    ...
    pub sweep_status: AbsoluteFinalReadinessTerminalSweepStatusV1,
    pub sweep_digest: String,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ReadinessTerminalSweepReportV1 {
    pub schema_version: u16,
    pub sweep: AbsoluteFinalReadinessTerminalSweepV1,
    pub consumers: Vec<ReadinessTerminalConsumerStatusV1>,
}

pub fn readiness_terminal_sweep(
    workdir: &Path,
    out: &Path,
) -> Result<ReadinessTerminalSweepReportV1, OpsError> {
    ...
    let active = models_active_review_snapshot(
        workdir,
        &workdir.join("out/active_review_snapshot_readiness_terminal_sweep.json"),
    )?;
    ...
    let final_readiness = final_readiness_consumer_sweep(
        workdir,
        &workdir.join("out/final_readiness_consumer_sweep_readiness_terminal_sweep.json"),
    )?;
    let residual = readiness_residual_sweep(
        workdir,
        &workdir.join("out/readiness_residual_sweep_readiness_terminal_sweep.json"),
    )?;
    let residual_free = residual_free_readiness_sweep(
        workdir,
        &workdir.join("out/residual_free_readiness_sweep_readiness_terminal_sweep.json"),
    )?;
    let absolute = readiness_absolute_sweep(
        workdir,
        &workdir.join("out/readiness_absolute_sweep_readiness_terminal_sweep.json"),
    )?;

    let authority_ctx = require_absolute_final_readiness_terminal_inputs(
        &truths,
        Some(&reduction),
        &applied,
        &entry,
    ...
    let sweep_status = if consumers.iter().any(|c| {
        ...
        AbsoluteFinalReadinessTerminalSweepStatusV1::Fail
    };
    ...
    let report = ReadinessTerminalSweepReportV1 {
        schema_version: 1,
        sweep,
        consumers,
    };
...
fn check_consumer(
    ...
    workdir: &Path,
    rel_path: &str,
    authority_ctx: &crate::AbsoluteFinalReadinessTerminalInputsV1,
) -> Result<ReadinessTerminalConsumerStatusV1, OpsError> {
    let mut mismatch_categories = BTreeSet::new();
    let path = workdir.join(rel_path);
    if !path.exists() {
        mismatch_categories
            .insert(ReadinessTerminalMismatchCategoryV1::ConsumerSkippedTerminalReadinessInputs);
        return Ok(ReadinessTerminalConsumerStatusV1 {
            consumer: consumer.to_string(),
            status: AbsoluteFinalReadinessTerminalSweepStatusV1::LegacyPresent,
    ...
    if !field_match("applied_supported_set_digest_prefix", expected_scope) {
        mismatch_categories
            .insert(ReadinessTerminalMismatchCategoryV1::ReadinessInputScopeMismatch);
    }
    if !field_match("canonical_readiness_spine_digest_prefix", expected_spine) {
        mismatch_categories
            .insert(ReadinessTerminalMismatchCategoryV1::ReadinessInputSpineMismatch);
    }
    if !field_match(
    ...
    if !field_match("readiness_residual_sweep_digest_prefix", expected_residual) {
        mismatch_categories.insert(ReadinessTerminalMismatchCategoryV1::ReadinessEchoPathPresent);
    }
    if !field_match(
        ...
        mismatch_categories
            .insert(ReadinessTerminalMismatchCategoryV1::ConsumerUsedReadinessEchoPath);
    }
    ...
    let status = if mismatch_categories.is_empty() {
        ...
        .contains(&ReadinessTerminalMismatchCategoryV1::ReadinessEchoPathPresent)
        || mismatch_categories
            .contains(&ReadinessTerminalMismatchCategoryV1::ConsumerUsedReadinessEchoPath)
    {
        AbsoluteFinalReadinessTerminalSweepStatusV1::LegacyPresent
    } else {
        AbsoluteFinalReadinessTerminalSweepStatusV1::Fail
    ...
    Ok(ReadinessTerminalConsumerStatusV1 {
        consumer: consumer.to_string(),
        status,
        mismatch_categories: mismatch_categories.into_iter().collect(),
    })
...
fn derive_sweep(
    ctx: &crate::AbsoluteFinalReadinessTerminalInputsV1,
    covered_consumer_count: u16,
    residual_path_count: u16,
    sweep_status: AbsoluteFinalReadinessTerminalSweepStatusV1,
) -> AbsoluteFinalReadinessTerminalSweepV1 {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(b"absolute_final_readiness_terminal_sweep_v1");
...
mod tests {
    ...
    #[test]
    fn readiness_terminal_sweep_digest_stable() {
        let ctx = crate::AbsoluteFinalReadinessTerminalInputsV1 {
