...
use crate::{
    artifact_schema, bundle_final_consolidation_sweep,
    canonical_final_consolidation_continuity_sweep, check_artifact_schema_snapshots, docs_lint,
    governance_final_consolidation_sweep, load_applied_supported_set_context_v1,
    models_consistency_check, portability_check, primary_semantics_final_consolidation_sweep,
    readiness_final_consolidation_sweep, v0_gate, v10_gate, v11_gate, v12_gate, v13_gate, v14_gate,
    v15_gate, v16_gate, v17_gate, v1_gate, v2_gate, v3_gate, v4_gate, v5_gate, v6_gate, v7_gate,
    v8_gate, v9_gate, BundleFinalConsolidationStatusV1,
    CanonicalFinalConsolidationContinuityStatusV1, DocsLintArgs, DocsLintMode, GateStatus,
    GovernanceFinalConsolidationStatusV1, OpsError, PrimarySemanticsFinalConsolidationStatusV1,
    ReadinessFinalConsolidationStatusV1, SupportedScopeExecutionV13, V0GateOverallStatus,
...
pub enum V18GateOverallStatus {
    Pass,
    Fail,
}

#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V18GateCheckV1 {
    pub name: String,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct V18GateReportV1 {
    pub schema_version: u16,
    pub overall_status: V18GateOverallStatus,
    pub checks: Vec<V18GateCheckV1>,
}

pub fn v18_gate(workdir: &Path, out: &Path) -> Result<V18GateReportV1, OpsError> {
    ...
    let v0 = v0_gate(
        ...
        &workdir.join("out/v0_gate_report_v18_gate.json"),
    )?;
    checks.push(v18_gate_check(
        "v0_gate_pass",
    ...
    let v1 = v1_gate(workdir, &workdir.join("out/v1_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v1_gate_pass",
    ...
    let v2 = v2_gate(workdir, &workdir.join("out/v2_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v2_gate_pass",
    ...
    let v3 = v3_gate(workdir, &workdir.join("out/v3_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v3_gate_pass",
    ...
    let v4 = v4_gate(workdir, &workdir.join("out/v4_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v4_gate_pass",
    ...
    let v5 = v5_gate(workdir, &workdir.join("out/v5_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v5_gate_pass",
    ...
    let v6 = v6_gate(workdir, &workdir.join("out/v6_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v6_gate_pass",
    ...
    let v7 = v7_gate(workdir, &workdir.join("out/v7_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v7_gate_pass",
    ...
    let v8 = v8_gate(workdir, &workdir.join("out/v8_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v8_gate_pass",
    ...
    let v9 = v9_gate(workdir, &workdir.join("out/v9_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v9_gate_pass",
    ...
    let v10 = v10_gate(workdir, &workdir.join("out/v10_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v10_gate_pass",
    ...
    let v11 = v11_gate(workdir, &workdir.join("out/v11_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v11_gate_pass",
    ...
    let v12 = v12_gate(workdir, &workdir.join("out/v12_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v12_gate_pass",
    ...
    let v13 = v13_gate(workdir, &workdir.join("out/v13_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v13_gate_pass",
    ...
    let v14 = v14_gate(workdir, &workdir.join("out/v14_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v14_gate_pass",
    ...
    let v15 = v15_gate(workdir, &workdir.join("out/v15_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v15_gate_pass",
    ...
    let v16 = v16_gate(workdir, &workdir.join("out/v16_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v16_gate_pass",
    ...
    let v17 = v17_gate(workdir, &workdir.join("out/v17_gate_report_v18_gate.json"))?;
    checks.push(v18_gate_check(
        "v17_gate_pass",
    ...
    let governance = governance_final_consolidation_sweep(
        workdir,
        &workdir.join("out/governance_final_consolidation_sweep_v18_gate.json"),
    )?;
    checks.push(v18_gate_check(
        "governance_final_consolidation_pass",
        gate_from_bool(matches!(
            governance.sweep.consolidation_status,
            GovernanceFinalConsolidationStatusV1::Pass
        )),
        [(
            "governance_final_consolidation_sweep".to_string(),
            digest_prefix(&governance)?,
        )],
        "REMEDIATE_GOVERNANCE_FINAL_CONSOLIDATION_SWEEP",
        "NOTE_REQUIRED_GOVERNANCE_FINAL_CONSOLIDATION",
    ));

    let supported_scope_execution_path = workdir.join("out/supported_scope_execute_v13.json");
    let supported_scope_execution =
        load_supported_scope_execution_v13(&supported_scope_execution_path)?;
    checks.push(v18_gate_check(
        "supported_scope_execution_v13_present",
        gate_from_bool(supported_scope_execution.is_some()),
        [(
            "supported_scope_execution_v13".to_string(),
            file_digest_or_missing(&supported_scope_execution_path)?,
        )],
        "REMEDIATE_RUN_SUPPORTED_SCOPE_EXECUTION_V13",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V13",
    ));
    ...
    let supported_scope_consistent = supported_scope_execution.as_ref().is_some_and(|exec| {
        exec.resulting_supported_set_digest_prefix == applied_scope.applied_set_digest_prefix
            ...
            && exec.governance_final_consolidation_sweep_digest_prefix
                == prefix_hex(&governance.sweep.consolidation_digest)
    });
    checks.push(v18_gate_check(
        "supported_scope_execution_v13_consistent",
        ...
        "REMEDIATE_SUPPORTED_SCOPE_EXECUTION_V13_STALE",
        "NOTE_REQUIRED_SUPPORTED_SCOPE_EXECUTION_V13_CURRENT",
    ));

    let readiness = readiness_final_consolidation_sweep(
        workdir,
        &workdir.join("out/readiness_final_consolidation_sweep_v18_gate.json"),
    )?;
    checks.push(v18_gate_check(
        "readiness_final_consolidation_pass",
        gate_from_bool(matches!(
            readiness.sweep.consolidation_status,
            ReadinessFinalConsolidationStatusV1::Pass
        )),
        [(
            "readiness_final_consolidation_sweep".to_string(),
            digest_prefix(&readiness)?,
        )],
        "REMEDIATE_READINESS_FINAL_CONSOLIDATION_SWEEP",
        "NOTE_REQUIRED_READINESS_FINAL_CONSOLIDATION",
    ));

    let bundle = bundle_final_consolidation_sweep(
        workdir,
        &workdir.join("out/bundle_final_consolidation_sweep_v18_gate.json"),
    )?;
    checks.push(v18_gate_check(
        "bundle_final_consolidation_pass",
        gate_from_bool(matches!(
            bundle.sweep.consolidation_status,
            BundleFinalConsolidationStatusV1::Pass
        )),
        [(
            "bundle_final_consolidation_sweep".to_string(),
            digest_prefix(&bundle)?,
        )],
        "REMEDIATE_BUNDLE_FINAL_CONSOLIDATION_SWEEP",
        "NOTE_REQUIRED_BUNDLE_FINAL_CONSOLIDATION",
    ));

    let semantics = primary_semantics_final_consolidation_sweep(
        workdir,
        &workdir.join("out/primary_semantics_final_consolidation_sweep_v18_gate.json"),
    )?;
    checks.push(v18_gate_check(
        "primary_semantics_final_consolidation_pass",
        gate_from_bool(matches!(
            semantics.sweep.consolidation_status,
            PrimarySemanticsFinalConsolidationStatusV1::Pass
        )),
        [(
            "primary_semantics_final_consolidation_sweep".to_string(),
            digest_prefix(&semantics)?,
        )],
        "REMEDIATE_PRIMARY_SEMANTICS_FINAL_CONSOLIDATION_SWEEP",
        "NOTE_REQUIRED_PRIMARY_SEMANTICS_FINAL_CONSOLIDATION",
    ));
    ...
    let continuity = continuity_bundle
        ...
        .map(|bundle_path| {
            canonical_final_consolidation_continuity_sweep(
                workdir,
                bundle_path,
                &workdir.join("out/canonical_final_consolidation_continuity_sweep_v18_gate.json"),
            )
        })
        .transpose()?;
    ...
    checks.push(v18_gate_check(
        "sole_final_consolidated_canonical_top_level_continuity_proof_pass",
        continuity_status,
        [(
        ...
        "REMEDIATE_CANONICAL_FINAL_CONSOLIDATION_CONTINUITY",
        "NOTE_REQUIRED_CANONICAL_FINAL_CONSOLIDATION_CONTINUITY",
    ));
    ...
    let portability = portability_check(&workdir.join("out/portability_check_v18_gate.json"))?;
    checks.push(v18_gate_check(
    ...
    let model_consistency = models_consistency_check(
        workdir,
        &workdir.join("out/models_consistency_check_v18_gate.json"),
    )?;
...
fn legacy_status_from_governance(
    report: &crate::GovernanceFinalConsolidationSweepReportV1,
) -> GateStatus {
    if matches!(
...
fn legacy_status_from_readiness(
    report: &crate::ReadinessFinalConsolidationSweepReportV1,
) -> GateStatus {
    if matches!(
        report.sweep.consolidation_status,
        ReadinessFinalConsolidationStatusV1::LegacyPresent
...
fn legacy_status_from_bundle(report: &crate::BundleFinalConsolidationSweepReportV1) -> GateStatus {
    if matches!(
        report.sweep.consolidation_status,
        BundleFinalConsolidationStatusV1::LegacyPresent
    ) {
...
fn load_supported_scope_execution_v13(
    path: &Path,
) -> Result<Option<SupportedScopeExecutionV13>, OpsError> {
    if !path.exists() {
        return Ok(None);
    }
...
fn overall_from_checks(checks: &[V18GateCheckV1]) -> V18GateOverallStatus {
    if checks
        .iter()
        .all(|check| matches!(check.status, GateStatus::Pass | GateStatus::Skip))
    {
        V18GateOverallStatus::Pass
    } else {
        V18GateOverallStatus::Fail
    }
}
...
mod tests {
    ...
    #[test]
    fn v18_gate_check_order_is_fixed() {
        let checks = vec![
            "v0_gate_pass",
            "v1_gate_pass",
            "v2_gate_pass",
        ...
        let report = V18GateReportV1 {
            schema_version: 1,
            overall_status: V18GateOverallStatus::Pass,
            checks: checks
                .iter()
                .map(|name| v18_gate_check(name, GateStatus::Pass, [], "REMEDIATE", "NOTE"))
                .collect(),
        };
    ...
    #[test]
    fn v18_gate_report_serialization_is_deterministic() {
        let report = V18GateReportV1 {
            schema_version: 1,
            overall_status: V18GateOverallStatus::Pass,
            checks: vec![
    ...
    #[test]
    fn v18_gate_normalization_is_fail_closed() {
        let checks = vec![
            v18_gate_check("required", GateStatus::Pass, [], "R", "N"),
            v18_gate_check("required_fail", GateStatus::Fail, [], "R", "N"),
            v18_gate_check("optional_skip", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
    ...
    #[test]
    fn v18_gate_normalization_supports_optional_skip_on_pass_path() {
        let checks = vec![
            v18_gate_check("required_a", GateStatus::Pass, [], "R", "N"),
            v18_gate_check("required_b", GateStatus::Pass, [], "R", "N"),
            v18_gate_check("optional", GateStatus::Skip, [], "R", "N"),
        ];
        assert!(matches!(
            overall_from_checks(&checks),
            V18GateOverallStatus::Pass
    ...
    #[test]
    fn supported_scope_execution_v13_missing_is_none() {
        let temp = tempfile::tempdir().expect("tmp");
        let missing = temp.path().join("supported_scope_execute_v13.json");
        let loaded = load_supported_scope_execution_v13(&missing).expect("load");
        assert!(loaded.is_none());
    }

    #[test]
    fn required_final_consolidation_checks_fail_normalization_cases() {
        for required in [
            "supported_scope_execution_v13_present",
            "readiness_final_consolidation_pass",
            "bundle_final_consolidation_pass",
            ...
            let checks = vec![
                v18_gate_check("v17_gate_pass", GateStatus::Pass, [], "R", "N"),
                v18_gate_check(required, GateStatus::Fail, [], "R", "N"),
                v18_gate_check(
                    "optional_backend_path_consistent",
                    GateStatus::Skip,
                    [],
