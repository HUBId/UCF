...
pub enum TerminalAbsoluteFinalInputContinuityStatusV1 {
    Pass,
    Fail,
    LegacyPresent,
}
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct TerminalAbsoluteFinalInputContinuityAuthorityV1 {
    ...
    pub absolute_final_input_continuity_authority_digest_prefix: String,
    pub continuity_status: TerminalAbsoluteFinalInputContinuityStatusV1,
    pub blocking_codes: Vec<String>,
    pub remediation_codes: Vec<String>,
    pub authority_digest: String,
}

pub fn terminal_absolute_final_input_continuity_sweep(
    workdir: &Path,
    bundle: &Path,
    out: &Path,
) -> Result<TerminalAbsoluteFinalInputContinuityAuthorityV1, OpsError> {
    ...
    let active = models_active_review_snapshot(
        workdir,
        &workdir
            .join("out/active_review_snapshot_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    let surfaces =
    ...
    let governance_absolute = governance_absolute_sweep(
        workdir,
        &workdir.join(
            "out/governance_absolute_sweep_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;
    let governance_terminal = governance_terminal_sweep(
        workdir,
        &workdir.join(
            "out/governance_terminal_sweep_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;
    ...
    let readiness_absolute = readiness_absolute_sweep(
        workdir,
        &workdir.join(
            "out/readiness_absolute_sweep_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;
    let readiness_terminal = readiness_terminal_sweep(
        workdir,
        &workdir.join(
            "out/readiness_terminal_sweep_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;
    ...
    let bundle_absolute = bundle_absolute_sweep(
        workdir,
        &workdir
            .join("out/bundle_absolute_sweep_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    let bundle_terminal = bundle_terminal_sweep(
        workdir,
        &workdir
    ...
    let primary_absolute = primary_semantics_absolute_sweep(
        workdir,
        &workdir.join("out/primary_semantics_absolute_sweep_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    let primary_terminal = primary_semantics_terminal_sweep(
        workdir,
        &workdir.join("out/primary_semantics_terminal_sweep_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    ...
    let review_packet = operator_review_packet(
        ...
        &workdir
            .join("out/operator_review_packet_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    let signoff = operator_signoff(
        ...
        &workdir.join("out/operator_signoff_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    let workflow = operator_workflow_chain(
        ...
        &workdir.join(
            "out/operator_workflow_chain_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;
    let roundtrip = operator_roundtrip_chain_check(
        ...
        bundle,
        &workdir.join(
            "out/operator_roundtrip_chain_terminal_absolute_final_input_continuity_sweep.json",
        ),
    )?;

    let absolute_final = absolute_final_input_continuity_sweep(
        workdir,
        bundle,
        &workdir.join("out/absolute_final_input_continuity_sweep_terminal_absolute_final_input_continuity_sweep.json"),
    )?;
    ...
    if governance_terminal
        .sweep
        .applied_supported_set_digest_prefix
        != *expected_applied
        || readiness_terminal.sweep.applied_supported_set_digest_prefix != *expected_applied
        || bundle_terminal.sweep.applied_supported_set_digest_prefix != *expected_applied
    ...
    {
        blocking.insert("TERMINAL_FINAL_INPUT_SCOPE_MISMATCH");
    ...
    if governance_terminal
        ...
        || !matches!(
            governance_terminal.sweep.sweep_status,
    ...
    {
        blocking.insert("TERMINAL_FINAL_INPUT_GOVERNANCE_MISMATCH");
        remediation.insert("run_governance_terminal_sweep");
    }

    if !matches!(
        readiness_terminal.sweep.sweep_status,
        ...
        blocking.insert("TERMINAL_FINAL_INPUT_READINESS_MISMATCH");
        remediation.insert("run_readiness_terminal_sweep");
    }

    if !matches!(
        primary_terminal.sweep.sweep_status,
        ...
        blocking.insert("TERMINAL_FINAL_INPUT_PRIMARY_SEMANTICS_MISMATCH");
        remediation.insert("run_primary_semantics_terminal_sweep");
    }

    if !workflow.blocking_codes.is_empty() {
        blocking.insert("TERMINAL_FINAL_INPUT_WORKFLOW_MISMATCH");
    ...
    if !matches!(
        ...
        bundle_terminal.sweep.sweep_status,
        ...
        blocking.insert("TERMINAL_FINAL_INPUT_BUNDLE_MISMATCH");
        remediation.insert("run_bundle_terminal_sweep");
    }

    let residual_dependency_present = governance_terminal.sweep.residual_path_count > 0
        || readiness_terminal.sweep.residual_path_count > 0
        || bundle_terminal.sweep.residual_path_count > 0
        || primary_terminal.sweep.residual_path_count > 0;
    ...
    let legacy_present = matches!(
        ...
        absolute_final.continuity_status,
    ...
    let mut report = TerminalAbsoluteFinalInputContinuityAuthorityV1 {
        schema_version: 1,
        ...
        absolute_final_governance_terminal_sweep_digest_prefix: prefix_hex(
            &governance_terminal.sweep.sweep_digest,
            DIGEST_PREFIX_LEN,
        ),
        canonical_readiness_spine_digest_prefix: readiness_terminal
            .sweep
        ...
        absolute_final_readiness_terminal_sweep_digest_prefix: prefix_hex(
            &readiness_terminal.sweep.sweep_digest,
            DIGEST_PREFIX_LEN,
        ),
        canonical_bundle_spine_digest_prefix: bundle_terminal
            .sweep
        ...
        absolute_final_bundle_terminal_sweep_digest_prefix: prefix_hex(
        ...
        absolute_final_primary_semantics_terminal_sweep_digest_prefix: prefix_hex(
            &primary_terminal.sweep.sweep_digest,
            DIGEST_PREFIX_LEN,
        ),
        ...
        absolute_final_input_continuity_authority_digest_prefix: prefix_hex(
            &absolute_final.authority_digest,
            DIGEST_PREFIX_LEN,
        ),
        continuity_status: if legacy_present {
            ...
            TerminalAbsoluteFinalInputContinuityStatusV1::Fail
        },
    ...
    report.authority_digest = terminal_absolute_final_input_continuity_digest(&report)?;
...
fn terminal_absolute_final_input_continuity_digest(
    report: &TerminalAbsoluteFinalInputContinuityAuthorityV1,
...
mod tests {
    ...
    #[test]
    fn terminal_absolute_final_input_continuity_digest_stable() {
        let mut report = TerminalAbsoluteFinalInputContinuityAuthorityV1 {
            schema_version: 1,
            applied_supported_set_digest_prefix: "aa".repeat(8),
            canonical_governance_entry_digest_prefix: "bb".repeat(8),
            residual_free_governance_absolute_sweep_digest_prefix: "cc".repeat(8),
            absolute_final_governance_terminal_sweep_digest_prefix: "dd".repeat(8),
            canonical_readiness_spine_digest_prefix: "ee".repeat(8),
            residual_free_readiness_absolute_sweep_digest_prefix: "ff".repeat(8),
            absolute_final_readiness_terminal_sweep_digest_prefix: "11".repeat(8),
            ...
            absolute_final_input_continuity_authority_digest_prefix: "cc".repeat(8),
            continuity_status: TerminalAbsoluteFinalInputContinuityStatusV1::Pass,
            blocking_codes: Vec::new(),
            remediation_codes: Vec::new(),
            authority_digest: String::new(),
        };
        report.authority_digest =
            terminal_absolute_final_input_continuity_digest(&report).expect("digest");
        let stable = terminal_absolute_final_input_continuity_digest(&report).expect("stable");
