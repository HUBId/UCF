...
use ucf_ops::{
    ...
    bundle_final_consolidation_sweep, bundle_residual_sweep, bundle_seal_sweep,
    bundle_stabilization_sweep, bundle_terminal_sweep, bundle_ultimate_sweep,
    canonical_closure_continuity_sweep, canonical_convergence_continuity_sweep,
    canonical_final_consolidation_continuity_sweep, canonical_seal_continuity_sweep,
    ...
    governance_lock_sweep, governance_residual_sweep, governance_seal_sweep,
    ...
    primary_semantics_absolute_sweep, primary_semantics_closure_sweep,
    primary_semantics_convergence_sweep, primary_semantics_final_consolidation_sweep,
    primary_semantics_residual_sweep, primary_semantics_seal_sweep,
    ...
    readiness_gate, readiness_lock_sweep, readiness_residual_sweep, readiness_seal_sweep,
    readiness_spine_check, readiness_spine_sweep, readiness_stabilization_sweep,
    readiness_terminal_sweep, readiness_ultimate_sweep, release_build_rc, release_rc1_gate,
    ...
    residual_free_primary_semantics_sweep, residual_free_readiness_sweep, review_truth_check,
    ...
    BundleConvergenceStatusV1, BundleFinalConsolidationStatusV1, BundleSealStatusV1,
    BundleStabilizationStatusV1, CanonicalBundleAuthorityStatusV2,
    CanonicalReadinessAuthorityStatusV2, CanonicalSealContinuityStatusV1, ChangeImpactArgs,
    ...
    GoldenVerifyReport, GovernanceClosureStatusV1, GovernanceConvergenceStatusV1,
    GovernanceEntryAuthorityStatusV2, GovernanceEntryCheckStatusV1,
    GovernanceFinalConsolidationStatusV1, GovernanceLockStatusV1, GovernanceResidualSweepStatusV1,
    GovernanceSealStatusV1, GovernanceStabilizationStatusV1, NightlySummarizeArgs,
    OperatorReportArgs, OperatorReviewPacketArgs, OperatorSignoffArgs, OperatorWorkflowArgs,
    PrimarySemanticsClosureStatusV1, PrimarySemanticsConvergenceStatusV1,
    PrimarySemanticsFinalConsolidationStatusV1, PrimarySemanticsSealStatusV1,
    PrimarySemanticsStabilizationStatusV1, ReadinessClosureStatusV1, ReadinessConvergenceStatusV1,
    ReadinessFinalConsolidationStatusV1, ReadinessLockStatusV1, ReadinessSealStatusV1,
    ...
    match cmd {
            ...
            match sub {
                ...
                "report" => {
                    ...
                    let has_fail = [
                        ...
                        &report.governance_closure_sweep_smoke,
                        &report.supported_scope_execute_v14_smoke,
                        &report.readiness_closure_sweep_smoke,
                        &report.bundle_closure_sweep_smoke,
                        &report.primary_semantics_closure_sweep_smoke,
        ...
        "models" => {
            ...
            match sub {
                ...
                "supported-scope-execute-v15" => {
                    let out = arg_value(&args, "--out")
                        .map(PathBuf::from)
                        .unwrap_or_else(|| PathBuf::from("./out/supported_scope_execute_v15.json"));
                    let report = models_supported_scope_execute_v15(&workdir, &out)?;
                    let applied = load_applied_supported_set_context_v1(&workdir)?;
                    println!("previous_applied_scope={}", applied.slots.join(","));
                    println!("decision={:?}", report.execution_decision);
                    if let Some(slot) = report.chosen_candidate_slot.as_ref() {
                ...
                _ => {
                    return Err(
                        "usage: ucf-ops models <verify|probe|stage|promote|rollback|list|active-check|active-evidence|eligibility|evidence-snapshot|active-review-snapshot|supported-set-review|supported-scope-reevaluate|supported-scope-execute|supported-scope-execute-v4|supported-scope-execute-v5|supported-scope-execute-v6|supported-scope-execute-v7|supported-scope-execute-v8|supported-scope-execute-v9|supported-scope-execute-v10|supported-scope-execute-v11|supported-scope-execute-v12|supported-scope-execute-v13|supported-scope-execute-v14|supported-scope-execute-v15|supported-set-apply|backend-resolution|consistency-check|applied-scope-check|recommend-rollback|shadow-ready> ..."
                            .into(),
                    )
                }
            }
        ...
        "canonical-seal-continuity-sweep" => {
            let bundle = arg_value(&args, "--bundle")
                .map(PathBuf::from)
                .ok_or_else(|| {
                    "usage: ucf-ops canonical-seal-continuity-sweep --bundle <path> --out ./out/canonical_seal_continuity_sweep.json".to_string()
                })?;
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/canonical_seal_continuity_sweep.json"));
            let report = canonical_seal_continuity_sweep(&workdir, &bundle, &out)?;
            println!("out={}", out.display());
            println!("continuity_status={:?}", report.continuity_status);
            if !report.blocking_codes.is_empty() {
                println!("blocking_codes={}", report.blocking_codes.join(","));
        ...
        "governance-seal-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/governance_seal_sweep.json"));
            let report = governance_seal_sweep(&workdir, &out)?;
            println!("out={}", out.display());
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                ...
                report.sweep.covered_consumer_count,
                report.sweep.residual_path_count,
                report.sweep.seal_digest
            );
            if !matches!(report.sweep.seal_status, GovernanceSealStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        "governance-lock-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/governance_lock_sweep.json"));
            let report = governance_lock_sweep(&workdir, &out)?;
            println!("out={}", out.display());
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                report.sweep.lock_status,
                report.sweep.covered_consumer_count,
                report.sweep.residual_path_count,
                report.sweep.lock_digest
            );
            if !matches!(report.sweep.lock_status, GovernanceLockStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        ...
        "readiness-seal-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/readiness_seal_sweep.json"));
            let report = readiness_seal_sweep(&workdir, &out)?;
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                report.sweep.seal_status,
                report.sweep.covered_consumer_count,
            ...
            println!("out={}", out.display());
            if !matches!(report.sweep.seal_status, ReadinessSealStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        "readiness-lock-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/readiness_lock_sweep.json"));
            let report = readiness_lock_sweep(&workdir, &out)?;
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                report.sweep.readiness_lock_status,
                report.sweep.covered_readiness_consumer_count,
                report.sweep.residual_readiness_path_count,
                report.sweep.readiness_lock_digest
            );
            println!("out={}", out.display());
            if !matches!(
                report.sweep.readiness_lock_status,
        ...
        "bundle-closure-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/bundle_closure_sweep.json"));
            let report = bundle_closure_sweep(&workdir, &out)?;
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                report.sweep.closure_status,
                report.sweep.covered_consumer_count,
            ...
            println!("out={}", out.display());
            if !matches!(report.sweep.closure_status, BundleClosureStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        ...
        "bundle-seal-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/bundle_seal_sweep.json"));
            let report = bundle_seal_sweep(&workdir, &out)?;
            println!(
                "status={:?} consumers={} residual_paths={} digest={}",
                report.sweep.seal_status,
                report.sweep.covered_consumer_count,
            ...
            println!("out={}", out.display());
            if !matches!(report.sweep.seal_status, BundleSealStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        ...
        "primary-semantics-closure-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/primary_semantics_closure_sweep.json"));
            let report = primary_semantics_closure_sweep(&workdir, &out)?;
            println!("out={}", out.display());
            println!("covered_surfaces={}", report.sweep.covered_surface_count);
            println!("residual_paths={}", report.sweep.residual_path_count);
            println!("closure_digest={}", report.sweep.closure_digest);
            if !matches!(
                report.sweep.closure_status,
        ...
        "primary-semantics-seal-sweep" => {
            let out = arg_value(&args, "--out")
                .map(PathBuf::from)
                .unwrap_or_else(|| PathBuf::from("./out/primary_semantics_seal_sweep.json"));
            let report = primary_semantics_seal_sweep(&workdir, &out)?;
            println!("out={}", out.display());
            println!("covered_surfaces={}", report.sweep.covered_surface_count);
            println!("residual_paths={}", report.sweep.residual_path_count);
            println!("seal_digest={}", report.sweep.seal_digest);
            if !matches!(report.sweep.seal_status, PrimarySemanticsSealStatusV1::Pass) {
                std::process::exit(2);
            }
        }
        "scope" => {
