...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cmd {
        ...
        _ => {
            eprintln!(
                "usage: ucf-ops <bringup|diag|health|diagnostics|export-bugreport|verify-bugreport|replay-bugreport|replay|metrics-snapshot|explain-tick|metrics|models|security|attest|repro|exports|readiness-gate|preflight|goldens|nightly|dev|troubleshoot|adversarial-run|out|release|bench|runs|status|strict|ess|ebm|drift|alerts|operator|policy|portability|spec|change-impact|soak|governance-surfaces-check|governance-entry-check|governance-entry-sweep|final-governance-consumer-sweep|final-readiness-consumer-sweep|final-bundle-consumer-sweep|final-continuity-sweep|remediation-consistency-check|remediation-interop-check|remediation-spine-check|primary-semantics-sweep|final-primary-semantics-sweep|interop|v0|v1|v2|v3|v4|v5|v6|v7|v8|v9|version> [--workdir <path>] [--bundle <path>]"
            );
            std::process::exit(1);
        }
    }
