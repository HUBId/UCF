...
| `ucf-geist` | `domains/geist/crates/ucf-geist` | `research-experimental` | `partial-prototype` | `partial` | Unit tests plus Prompt 56 docs-only record-authority inventory | Workspace tests | No | later | Geist SelfState, loop-state, ISM-store, policy-gate, sleep-coupling, and archive-append prototype surfaces exist; Prompt 56 selected docs-only bounded authority alignment and deferred candidate/audit/boundary code to Prompt 57. | Existing kernel/upsert/archive/sleep mutation surfaces are too broad for bounded post-Sleep Geist/ISM; Self/recursion/anchor claims exceed current minimal E2E proof and must not imply identity anchoring. | Use `docs/roadmap/geist_ism_record_authority_schema_alignment.md`; next allowed step is a pure deterministic Self-State projection candidate from bounded Sleep provenance only, with no runtime apply, ISM upsert, Evidence/Archive append, Policy mutation, Gateway action, or identity finalization. |
...
| `ucf-neuromod` | `domains/ucf-neuromod` | `spine-supporting` for the minimal envelope hook; `research-experimental` for broader scheduler/state APIs | `partial-prototype` | `partial` | Unit tests plus Minimal Spine envelope hook integration test | Workspace tests | No | optional | Neuromod now exposes a deterministic `MinimalSpineNeuromodEnvelope` derived from canonical Evidence/Archive/Protocol links with bounded metadata hints; existing v0 field/scheduler APIs remain broader experimental surfaces. | Minimal hook is metadata-only and does not prove a full metabolic layer, hormone scheduler, HPA/DBM integration, policy/output override, replay/consolidation trigger, Gateway write, or compute integration. | Keep the v1.4 hook as an optional derived view; defer full neuromod/metabolic scheduler to Spine v2. |
