...
| `ucf-sleep-coordinator` | `core/crates/ucf-sleep-coordinator` | `spine-supporting` for future bounded Sleep planning only; `research-experimental` for existing trigger/report runner wiring | `partial-prototype` | `functional-prototype` | In-source unit tests plus Prompt 47 authority alignment docs | Workspace tests | No | later | Existing WAL-style sleep trigger state, replay summary counters, and RSA sleep phase runner adapter are inventory/prototype surfaces; Prompt 47 confirms no canonical SleepPlan, SleepCycle, SleepApplied, or SleepBoundary records exist yet. | Existing runnable trigger/report paths can append sleep reports through RSA/archive prototypes and must not be read as post-Replay Sleep v1 authority, Geist/ISM integration, identity stabilization, Evidence/Archive append contract, Gateway visibility, or runtime scheduler/queue/worker readiness. | Use `docs/roadmap/sleep_record_authority_schema_alignment.md` before making Sleep claims; next authorized line is a pure deterministic SleepPlan candidate from bounded Replay metadata only. |
