...
| `ucf-replay` | `runtime/ucf-replay` | `operational` | `current-supporting` | `functional-prototype` | Golden replay tests plus Prompt 37 record-authority alignment docs | Workspace tests | Backend argument selects stub/candle/burn | optional | Deterministic replay fixture/report path and audit-only `ReplayPlan`/`ReplayReport`/`ReplayResult` surfaces; Prompt 37 classifies `ReplayToken` as a shared primitive and leaves scheduled/applied scheduler authority deferred. | Golden fixture uses stub backend; not proof of real compute, Replay Scheduler readiness, replay scheduling, applied replay, Sleep/Geist/ISM integration, Evidence/Archive replay appends, or identity finalization. | Optional validation lane for Minimal Spine; use `docs/roadmap/replay_record_authority_schema_alignment.md` before Prompt 38 token-builder work. |
