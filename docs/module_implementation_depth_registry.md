...
| `ucf-consolidation` | `domains/consolidation/crates/ucf-consolidation` | `spine-supporting` for the minimal hook; `research-experimental` for broader pipeline | `partial-prototype` | `partial` | Unit tests plus Minimal Spine micro-hook integration test | Workspace tests | No | optional | Consolidation now exposes a deterministic `MinimalSpineMicroMilestoneCandidate` derived from canonical Evidence/Archive/Protocol links; broader kernel still touches archive, bus, commit, consistency, IIT, influence, macro finalization, and replay. | Minimal hook is candidate-only and does not prove a full Micro→Meso→Macro pipeline, replay scheduler, Geist/ISM, neuromod, or compute integration. | Keep the v1.3 hook as an optional derived view; defer full consolidation pipeline to Spine v2. |
