...
| `ucf-neuromod` | `domains/ucf-neuromod` | `spine-supporting` for the minimal envelope hook; `research-experimental` for broader scheduler/state APIs | `partial-prototype` | `partial` | Unit tests plus Minimal Spine envelope hook integration test | Workspace tests | No | optional | Neuromod now exposes a deterministic `MinimalSpineNeuromodEnvelope` derived from canonical Evidence/Archive/Protocol links with bounded metadata hints; existing v0 field/scheduler APIs remain broader experimental surfaces. Prompt M1 metabolic/hormone inventory + boundary roadmap is documented at `docs/roadmap/metabolic_hormone_control_layer_roadmap_boundary_audit.md`. | Minimal hook is metadata-only and does not prove a full metabolic layer, hormone scheduler, HPA/DBM integration, policy/output override, replay/consolidation trigger, Gateway write, or compute integration. | Keep the v1.4 hook as an optional derived view; defer full neuromod/metabolic scheduler to Spine v2. |
