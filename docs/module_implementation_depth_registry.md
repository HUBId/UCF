...
| `ucf-neuromod` | `domains/ucf-neuromod` | `spine-supporting` for the minimal envelope hook; `research-experimental` for broader scheduler/state APIs | `partial-prototype` | `partial` | Unit tests plus Minimal Spine envelope hook integration test and targeted `HormoneStateV1` contract tests | Workspace tests | No | optional | Neuromod now exposes a deterministic `MinimalSpineNeuromodEnvelope` plus bounded fixed-point `HormoneStateV1`/`NormalizedHormoneLevelV1` contract in `src/hormone_state_v1.rs` with deterministic clamping/validation tests; existing v0 field/scheduler APIs remain broader experimental surfaces. | Hormone contract is state-only and does not yet implement update rules/modulation output, replay/sleep/geist mapping, or runtime authority. | Keep as bounded deterministic state contract; implement update rules in M3 before any integration claims. |
