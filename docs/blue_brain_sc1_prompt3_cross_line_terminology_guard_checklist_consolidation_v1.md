...
| `supporting affected surface` | Region-/Relation-/MD2-/Guard-Dokumente und `blue_brain_region_first_integration.rs` | Stützende Konsolidierung der bereits existierenden Semantik. | Zweite operative Wahrheit neben BR6/IR1/MD2/SC1. |
...
| `current model mode` | descriptive model-mode read only | keine Contract-Autorität, keine Modellplattform, kein zweiter Vertiefungskandidat, keine direct action/execution/retry/memory/compute/safety authority |
...
- MD2 bleibt genau die maintenance-gehärtete `Amygdala ↔ Thalamus`-Vertiefung; `current model mode` bleibt deskriptiv.
