...
Recommended next prompt: **UCF Prompt 59 — ISM Candidate Boundary Without Identity Finalization**.

Prompt 58 is complete as a verify-only audit contract. Prompt 59 should define an ISM candidate boundary that can reference a passing Geist projection audit without writing or upserting ISM state, creating an IdentityAnchor, finalizing identity, mutating Policy Ecology, appending Evidence/Archive records, exposing Gateway/action authority, calling `GeistKernel::ingest_macro`, or changing Minimal Spine v1.x.
