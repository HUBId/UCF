...
| 48 | Deterministic SleepPlan Candidate from Replay Boundary | Define and, only if authorized by Prompt 47, implement a deterministic candidate builder from Replay boundary metadata. | PASS ReplayAudit, optional ReplayAppliedBoundary, schedule/token/provenance digests, static config, deterministic ordering. | Candidate output is stable across repeated runs; negative tests reject failed audit and forbidden flags. | No replay execution, no scheduler, no sleep phase run, no archive append, no Geist/ISM write. |
| 49 | SleepPlan Verify-Only Audit Contract | Add a verify-only SleepPlan audit contract. | PASS/FAIL status, failure reasons, audit digest, candidate digest checks, Replay reference checks. | Audit is deterministic, read-only, and fails on mutation/apply/archive/geist/identity flags. | Verify-only; no apply, no report append, no scheduler, no Gateway write. |
...
- Are there existing SleepPlan/SleepApplied records? Current audit found no canonical SleepPlan, SleepCycle, SleepApplied, or SleepAppliedBoundary records; Prompt 47 should confirm and decide whether local wrappers are enough.
- Where should the SleepPlan builder live: `runtime/ucf-replay`, `core/crates/ucf-sleep-coordinator`, a new bounded runtime crate, or docs-only until schema authority is settled?
- Should Sleep consume `ReplayAppliedBoundary`, `ReplayAudit`, or both? This audit recommends PASS `ReplayAudit` as the verify-only prerequisite and optional `ReplayAppliedBoundary` as a local boundary reference only.
