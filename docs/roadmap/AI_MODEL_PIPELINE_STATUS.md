...
| `runtime/ucf-compute` | Full runtime pipeline surface: world model, SAE extractor, SSM, LFM, orchestration, model store (allowlist/hash/max_bytes/pinning), capability wiring, stage contracts; plus a minimal bounded in-memory compute service core (`compute_service`) with canonical job lifecycle + technical admission wrapper. Candle/Burn compute paths exist in runtime crate. | Several paths are intentionally bounded/degraded fixtures and controlled stubs (e.g., burn v0 skeleton behavior and fixture-driven kernels). | Distributed scheduling/orchestration and persistent job store are intentionally not implemented in this phase. |
...
5. **Bounded compute service hardening (next increment)**
   - Expand from the current minimal in-memory lifecycle/admission core toward scheduler + execution hardening without introducing non-canonical request/error paths.
