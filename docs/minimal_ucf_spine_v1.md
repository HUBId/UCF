...
| Post-v1.x bounded append/readback guard | Replay `Other(65)`, Sleep `Other(66)`, and Geist/ISM `Other(67)` append/readback contracts are documented separately as audit/provenance persistence only. | They do not change Minimal Spine v1.x, add runtime or identity semantics, expose Gateway authority, claim production readiness, or create a second event log. | `docs/roadmap/evidence_archive_append_contracts_roadmap_boundary_audit.md` |
