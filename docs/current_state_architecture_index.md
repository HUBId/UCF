...
Use this index as the reading order for roadmap prompts until the Module Implementation-Depth Registry and the Minimal UCF Spine Specification exist. Code and currently passing gates remain the final source of truth.
...
| Consolidation Kernel | current-supporting | `docs/architecture/COHERENCE_LOOP.md`, consolidation references in current docs | `domains/consolidation/crates/ucf-consolidation`, `core/crates/ucf-commit`, `core/crates/ucf-sleep-coordinator` | Blue-Brain consolidation refreshes | bounded retrieval/consolidation candidate docs | Historical consolidation closure interpreted as current implementation depth. |
...
| Replay / Goldens / Drift | current-operational | `docs/replay-harness.md`, `docs/replay_audit.md`, `docs/golden_update.md`, `docs/adversarial_harness.md` | `runtime/ucf-replay`, `crates/replay_executor`, `crates/replay_evidence`, test/golden fixtures | drift/nightly reports | none primary | Stale drift reports used as current proof. |
| Real Compute / AI Backends / Burn / Candle / Toy / Stub | experimental | `docs/feature_matrix.md`, `docs/backends.md`, `docs/models_eligibility_v3.md`, `docs/backend_evidence_snapshot_v4.md` | `runtime/ucf-compute`, `domains/ai*`, `runtime/ucf-ebm-train`, backend crates | real-compute readiness sweeps | `docs/real_compute_*`, `docs/backend_burn_*`, `docs/backend_candle_*` | Feature existence mistaken for enabled real-compute production lane. |
...
| Safety / Sandbox / Governor / Authority Boundaries | current-supporting | `docs/sandboxing-v1.md`, `docs/security_v1.md`, `docs/threat_model_v1.md`, `docs/no_hidden_network.md`, `docs/zero_trust_local.md` | `core/crates/ucf-sandbox`, `core/crates/ucf-risk-gate`, `runtime/ucf-policy`, governance crates | authority sweeps | research authority maps | Boundary docs read as capability docs. |
...
| `runtime/ucf-compute` | Compute/backends lane | experimental | no by default | Keep optional until real path is proven by enabled gates; toy/stub lanes are not production real compute. |
