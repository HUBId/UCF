...
Status: canonical HH-readiness Prompt 3 prerequisite and boundary map. This document mirrors `CANONICAL_BLUE_BRAIN_HH_PREREQUISITE_MAP` in `runtime/ucf-compute/src/blue_brain_region_first_integration.rs`; it is not a second truth source, not HH implementation, not a productive HH use, not a third model deepening, not a global HH platform, and not new Runtime/Selection/Reference/Execution authority.
...
## 3) Candidate prerequisites and boundaries

| Surface | Class | Current status | Later re-scope requirement |
| --- | --- | --- | --- |
| `candidate_relation_selected_and_bounded` | prerequisite already satisfied | `Basal Ganglia ↔ Cerebellum` is already the exactly-one first later-HH candidate and is relation-only. | Preserve one-candidate scope; do not open Cerebellum, Basal Ganglia or additional HH candidates. |
| `ir1_relation_surface_not_implemented` | prerequisite missing | IR1 keeps the candidate as an execution-interface-mediated architecture lane only and deferred/not-yet-implemented. | A separate architecture/fixture re-scope must exist before any HH diagnostic surface can be wired. |
| `candidate_input_contract_missing` | prerequisite missing | No canonical HH inputs exist for selection-channel suppression, timing/correction, excitability, threshold or refractory-shape probes. | Later inputs may only be deterministic simulation/diagnostic fixtures derived from approved relation evidence; never live action, execution, retry, memory or compute triggers. |
| `candidate_output_contract_missing` | prerequisite missing | No canonical HH outputs, fixtures, thresholds, caveats or consumer mappings exist. | Later outputs may be simulation-only diagnostic summaries; advisory use would require a separate bounded advisory contract and still no authority. |
| `hard_contract_boundary_hh_not_contract_state` | hard contract boundary | Canonical model-boundary rules keep model state outside Contract state and model output outside direct authority. | Any later HH output must down-map to diagnostics only unless separately bounded as advisory support without authority. |
| `hard_guard_boundary_no_direct_authority` | hard guard boundary | Existing Blue-Brain guard line forbids direct authority. | Later HH must fail closed on any attempted authority promotion. |
| `hard_runtime_selection_reference_boundary` | hard runtime/selection/reference boundary | Runtime has no HH productive read path; Selection cannot read HH as action authority; Reference/Context remains read/reference and non-mutative. | A later Runtime read may be bounded diagnostic/advisory-only after separate contract approval; Selection, Reference and Execution authority stay external to HH. |
| `simulation_only_hh_allowance` | simulation-only HH allowance | BB10 allows HH only as deferred diagnostic vocabulary and the candidate map keeps this relation later-HH/deferred. | Later HH evidence must remain deterministic, fixture-bounded, diagnostic-only and separated from productive runtime authority. |
| `forbidden_productive_hh_path` | forbidden productive HH path | HH is nowhere productive mode and this candidate relation is still deferred. | No later re-scope may silently convert HH diagnostics into productive action, execution, retry, memory, compute or safety behavior. |
| `non_canonical_internal_only_hh_paths` | non-canonical/internal-only HH path | Microcircuit, network simulation, CoreNEURON, Neurodamus and adjacent biophysical paths have no canonical consumer, contract support, runtime path or implementation lane here. | Keep these paths internal-only unless an explicit future repo decision creates a new scope outside this prerequisite line. |
| `kuramoto_deepening_interaction_boundary` | hard contract boundary | `Amygdala ↔ Thalamus` and `Amygdala ↔ Basal Ganglia` remain the only bounded Kuramoto-like advisory/diagnostic deepenings. | HH for `Basal Ganglia ↔ Cerebellum` must not substitute, extend or backdoor-promote those active deepenings. |
| `fixture_performance_determinism_prerequisites_missing` | prerequisite missing | No HH fixture corpus, golden comparison, fixed-point/canonical encoding rule or bounded performance budget is approved. | Later re-scope must provide deterministic fixtures and explicit budgets before any simulation-only diagnostic can be considered. |
| `no_scope_expansion_boundary` | hard guard boundary | This prompt only tightens prerequisites and boundaries for one relation candidate. | Future work must be a separate HH-readiness re-scope and cannot infer implementation permission from this map. |

## 4) Allowed and disallowed later inputs
...
## 5) Allowed and disallowed later outputs
...
## 6) Model and diagnostics boundaries
...
## 7) Runtime, selection, reference and execution boundaries
...
## 9) Why HH remains closed now

HH remains closed because the candidate relation is not implemented, the input and output contracts are absent, the fixture/performance/determinism envelope is absent, and all Runtime/Selection/Reference/Execution authority paths remain hard-bounded. The only accepted value of this line is prerequisite clarity for a possible later, separate, simulation-only/diagnostic-only re-scope.

## 10) Next HH-readiness steps
