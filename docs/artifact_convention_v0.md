...
The report includes bounded command-result tails plus `phase_timings[]` for workspace-test-check metadata, Cargo command, and report assembly diagnostics, and additive non-authoritative diagnostics (`last_phase`, `cargo_command_started_at_utc`, `cargo_command_duration_ms`, `diagnostic_notes`). These timing/diagnostic fields are informational only; they do not weaken freshness, command, dirty-state, or PASS validation.
